...
def cache_fn(f):
    ...
    @wraps(f)
    def cached_fn(*args, _cache=True, key=None,
                  **kwargs):  # args——位置参数  _cache——是否启用缓存，默认启用  key——缓存的索引键（由调用者决定）  **kwargs——关键字参数
...
⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐
我知道如何改了：
1. 直接在训练脚本里调用这个类的perceiver传递数据和hsiclasso获得的alpha
2. 修改Attention类的forward函数，里面根据context存在与否，决定是否使用掩码，掩码作用于sim上，就不会改变原始输入
    但是后期得看看如何更好地作用
3. 修改Perceiver类的forward函数，里面判断层数，如果是第一层，就使用传入的静态掩码alpha，否则，就使用DynamicMask类生成的动态掩码
4. 增加一个类DynamicMask，这样就可以和Perceiver模型联合训练，保证可微、可反向传播
